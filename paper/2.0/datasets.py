...
def get_all_files(root):
    ...
    for k, dset in enumerate(dsets):
        ...
        train_files = [
            tf for tf in train_files if tf[-10:] != f'_masks.png' and
            tf[-10:] != f'_masks.tif' and tf[-10:] != '_flows.tif'
        ]
        ...
        if k > 0:
            ...
            train_types = np.array([['cp', ctype[k]] for i in range(len(train_files))],
                                   dtype=object)
        ...
        test_files = [
            tf for tf in test_files if tf[-10:] != f'_masks.png' and
            tf[-10:] != f'_masks.tif' and tf[-10:] != '_flows.tif'
        ]
        ...
        if k > 0:
            ...
            test_types = np.array([['cp', ctype[k]] for i in range(len(test_files))],
                                  dtype=object)
...
def get_tissuenet_val(root, tissue_type='ALL', platform_type='ALL', nval=8, seed=1):
    ...
    labels = dat['y']
    if tissue_type != 'ALL':
        ix = (np.logical_and(dat['tissue_list'] == tissue_type,
                             dat['platform_list'] == platform_type)).nonzero()[0]
    X_val, y_val = data[ix], labels[ix]
    return X_val[:nval], y_val[:nval, :, :, [0]]


def get_tissuenet_train(root, tissue_type='ALL', platform_type='ALL', ntrain=10,
                        full_img=False, seed=1):
    train_files = glob(os.path.join(root, 'train_full/*.tif'))
    train_files = [
        tf for tf in train_files
...
def get_train_files(root, cell_type, tissue_type, platform_type, ntrain=10,
    ...
    if cell_type is not None:
        ...
        train_files = get_tissuenet_train(root, tissue_type, platform_type,
                                          ntrain=ntrain, full_img=full_img, seed=seed)
        channels = [2, 1]
