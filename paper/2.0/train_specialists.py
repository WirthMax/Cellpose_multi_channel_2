    ...
    for types, files in zip([train_types, test_types], [train_files, test_files]):
        ...
        for ibatch in trange(0, nimg, batch_size):
            ...
            imgi, lbl, scale = transforms.random_rotate_and_resize(
                imgs, Y=None, rescale=rsc, scale_range=0., random_crop=False,
                random_rotation=False, xy=(224, 224))
...
def cluster_styles(train_files, train_types, train_styles, test_files, test_types,
                   test_styles):
    from openTSNE import TSNE
    from sklearn.decomposition import PCA

    ctypes_train = -1 * np.ones(len(train_styles), 'int')
    ctypes_train[np.array([(train_types[i][0] == 'cp') * (train_types[i][1] == 'cyto')
                           for i in range(len(train_types))]).astype(bool)] = 0
    ctypes_train[np.array([train_types[i][0] == 'lc' for i in range(len(train_types))
                          ]).astype(bool)] = 1
    ctypes_train[np.array([train_types[i][0] == 'tn' for i in range(len(train_types))
                          ]).astype(bool)] = 2

    ctypes_test = -1 * np.ones(len(test_styles), 'int')
    ctypes_test[np.array([(test_types[i][0] == 'cp') * (test_types[i][1] == 'cyto')
                          for i in range(len(test_types))]).astype(bool)] = 0
    ctypes_test[np.array([test_types[i][0] == 'lc' for i in range(len(test_types))
                         ]).astype(bool)] = 1
    ctypes_test[np.array([test_types[i][0] == 'tn' for i in range(len(test_types))
                         ]).astype(bool)] = 2
    ...
    classifier = KNeighborsClassifier(n_neighbors=5).fit(train_styles_sub,
                                                         leiden_labels)
...
def train_style_net(root, style_path, pretrained_model=None, diam_mean=30.,
                    batch_size=8, flow_threshold=0.4, weight_decay=1e-5, n_epochs=400,
                    learning_rate=0.2, istyle=0, save_results=True, use_gpu=True):
    ...
    if train_files[0][:len(root)] != root:
        for i in range(len(train_files)):
            orig_root = os.path.split(
                os.path.split(os.path.split(train_files[i])[0])[0])[0]
        ...
        for i in range(len(test_files)):
            orig_root = os.path.split(
                os.path.split(os.path.split(test_files[i])[0])[0])[0]
    ...
    model_path = model.train(train_istyle, lbls_istyle, train_files=None,
                             channels=[1, 2], normalize=True, save_path=save_path,
                             ...
                             min_train_masks=10, netstr=netstr)
...
def train_general_net(
    ...
    if not style_sampling:
        ...
        ctypes[np.array([(train_types[i][0] == 'cp') * (train_types[i][1] == 'cyto')
                         for i in range(len(train_types))]).astype(bool)] = 0
        ctypes[np.array([train_types[i][0] == 'lc' for i in range(len(train_types))
                        ]).astype(bool)] = 1
        ctypes[np.array([train_types[i][0] == 'tn' for i in range(len(train_types))
                        ]).astype(bool)] = 2
    ...
    for iepoch in range(n_epochs):
        ...
        for k in range(0, nimg_per_epoch, batch_size):
            ...
            imgi, lbl, scale = transforms.random_rotate_and_resize(
                imgs, Y=lbls, rescale=rsc, scale_range=.5)
        ...
        lavg = lavg / len(rperm)
        print(
            f'{iepoch}, train_loss= {lavg:.4f}, LR={LR[iepoch]:.4f}, time {time.time()-t0:.2f}s'
        )
...
def eval_model(model_path, test_files, test_types, use_gpu=True, flow_threshold=0.4,
               diameter=30.):
    ...
    masks, flows, styles = model.eval(test_data, channels=[1, 2], rescale=rescale,
                                      resample=True, net_avg=False, batch_size=32,
                                      normalize=False, tile_overlap=0.1,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root', default=[], type=str,
                        help='root folder containing data to train and test on.')
    parser.add_argument('--style_path', default=[], type=str, help='style data path')
    parser.add_argument('--style_type', default=0, type=int,
                        help='style type to run net on')
    ...
    parser.add_argument('--diam_mean', default=30, type=int,
                        help='mean diameter to resize cells to')

    args = parser.parse_args()
    logger, log_path = logger_setup()

    if not args.general:
        train_style_net(args.root, args.style_path, istyle=args.style_type,
                        use_gpu=args.use_gpu, diam_mean=args.diam_mean,
