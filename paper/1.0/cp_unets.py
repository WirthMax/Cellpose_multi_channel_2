...
def get_pretrained_models(model_root, unet=0, nclass=3, residual=1, style=1,
                          concatenate=0):
    sstr = ['off', 'on']
...
def make_kfold_data(data_root):
    ...
    flow_labels = [
        os.path.join(train_root, '%03d_img_flows.tif' % i) for i in range(ntrain)
    ]
    ...
    flow_labels = [
        os.path.join(test_root, '%03d_img_flows.tif' % i) for i in range(ntest)
    ]

    all_inds = np.hstack(
        (np.random.permutation(ntrain), np.random.permutation(ntest) + ntrain))
    for j in range(9):
        ...
        test_inds = all_inds[np.arange(j * ntest + ntrain,
                                       (j + 1) * ntest + ntrain, 1, int) % ntot]
        for i, ind in enumerate(train_inds):
            ...
            shutil.copyfile(flow_labels[ind],
                            os.path.join(root_train, '%03d_img_flows.tif' % i))
        for i, ind in enumerate(test_inds):
            ...
            shutil.copyfile(flow_labels[ind],
                            os.path.join(root_test, '%03d_img_flows.tif' % i))
...
def train_unets(data_root):
    ...
    train_labels = [
        io.imread(os.path.join(train_root, '%03d_masks.tif' % i))[0] for i in range(ntrain)
    ]
    ...
    for k in range(len(concatenation)):
        # 4 nets for each
        for l in range(4):
            model = models.UnetModel(device=device, pretrained_model=None, diam_mean=30,
                                     residual_on=residual_on[k], style_on=style_on[k],
                                     concatenation=concatenation[k],
                                     nclasses=nclasses[k])
            model.train(train_data, train_labels, test_data, test_labels,
                        channels=channels, rescale=True, save_path=train_root)
...
def test_unets_main(data_root, save_root):
    ...
    for model_type in model_types:
        ...
        test_root = os.path.join(data_root,
                                 'images_%s/test/' % model_type.split('_')[0])
...
def test_unets(model_root, test_root, save_root, model_type='cyto'):
    ...
    for k in range(1):  #len(concatenation)):
        ...
        masks = model.eval(test_data, channels=channels, rescale=rescale,
                           net_avg=True)[0]
        ap = metrics.average_precision(test_labels, masks, threshold=thresholds)[0]
        print(ap[:, [0, 5, 8]].mean(axis=0))
        aps[k] = ap
        np.save(
            os.path.join(
                save_root, 'unet%d_residual_%s_style_%s_concatenation_%s_%s_masks.npy' %
...
def train_cellpose_nets(data_root):
    ...
    for j in range(9):
        ...
        train_labels = [
            io.imread(os.path.join(train_root, '%03d_masks.tif' % i))[0]
            for i in range(ntrain)
        ]
        train_flow_labels = [
            io.imread(os.path.join(train_root, '%03d_img_flows.tif' % i))[0]
            for i in range(ntrain)
        ]
        train_labels = [
            np.concatenate((train_labels[i][np.newaxis, :, :], train_flow_labels),
                           axis=0) for i in range(ntrain)
        ]
        ...
        test_labels = [
            io.imread(os.path.join(test_root, '%03d_masks.tif' % i))[0]
            for i in range(ntest)
        ]
        test_flow_labels = [
            io.imread(os.path.join(test_root, '%03d_img_flows.tif' % i))[0]
            for i in range(ntest)
        ]
        test_labels = [
            np.concatenate((test_labels[i][np.newaxis, :, :], test_flow_labels), axis=0)
            for i in range(ntest)
        ]
        ...
        for k in range(len(concatenation)):
            ...
            for l in range(4):
                model = models.CellposeModel(device=device, pretrained_model=None,
                                             diam_mean=30, residual_on=residual_on[k],
                                             style_on=style_on[k],
                                             concatenation=concatenation[k])
                model.train(images, labels, test_data=test_images,
                            test_labels=test_labels, channels=channels, rescale=True,
                            save_path=train_root)

                # train size network on default network once
                if k == 0 and l == 0:
                    sz_model = models.SizeModel(model, device=device)
                    sz_model.train(train_data, train_labels, test_data, test_labels,
                                   channels=channels)
                    ...
                    ccs = np.corrcoef(
                        diams_style,
                        np.array([utils.diameters(lbl)[0] for lbl in tlabels]))[0, 1]
                    cc = np.corrcoef(
                        predicted_diams,
                        np.array([utils.diameters(lbl)[0] for lbl in tlabels]))[0, 1]
                    print(
...
def test_cellpose_main(data_root, save_root):
    ...
    for model_type in model_types:
        if model_type == 'cyto' or model_type == 'nuclei':
            pretrained_models = [
                str(Path.home().joinpath('.cellpose/models/%s_%d' % (model_type, j)))
                for j in range(4)
            ]
        else:
            pretrained_models = glob(
                os.path.join(data_root, 'images_cyto_sp/train/models/cellpose_*'))
        test_root = os.path.join(data_root,
                                 'images_%s/test/' % model_type.split('_')[0])
...
def test_cellpose(test_root, save_root, pretrained_models, diam_file=None,
                  model_type='cyto'):
...
def test_nets_3D(stack, model_root, save_root, test_region=None):
    ...
    for m, model_arch in enumerate(model_archs):
        if model_arch == 'cellpose':
            ...
            masks = model.eval(stack, channels=[2, 1], rescale=30. / 25., do_3D=True,
                               min_size=2000)[0]
        else:
            pretrained_models = get_pretrained_models(model_root, unet=1,
                                                      nclass=int(model_arch[-1]),
                                                      residual=0, style=0,
                                                      concatenate=1)
            model = models.UnetModel(device=device, pretrained_model=pretrained_models)
            masks = model.eval(stack, channels=[2, 1], rescale=30. / 25., do_3D=True,
                               min_size=2000, cell_threshold=cell_thresholds[m],
...
def test_cellpose_kfold_aug(data_root, save_root):
    ...
    for j in range(9):
        ...
        pretrained_models = get_pretrained_models(model_root, 0, 3, residual_on[k],
                                                  style_on[k], concatenation[k])
        print(pretrained_models)
        ...
        masks = cp_model.eval(test_data, channels=channels, rescale=rescale,
                              net_avg=True, augment=True)[0]
        ap = metrics.average_precision(test_labels, masks, threshold=thresholds)[0]
        print(ap[:, [0, 5, 8]].mean(axis=0))
        aps[j] = ap
