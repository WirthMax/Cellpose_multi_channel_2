...
def cyto(test_root, save_root, save_figure=False):
    ...
    for m, model_type in enumerate(model_types):
        masks[m].append(
            np.load(os.path.join(save_root, 'cellpose_%s_masks.npy' % model_type),
                    allow_pickle=True))
        masks[m].append(
            np.load(os.path.join(save_root, 'maskrcnn_%s_masks.npy' % model_type),
                    allow_pickle=True))
        masks[m].append(
            np.load(os.path.join(save_root, 'stardist_%s_masks.npy' % model_type),
                    allow_pickle=True))
        ...
        masks[m].append(
            np.load(
                os.path.join(
                    save_root,
                    'unet3_residual_off_style_off_concatenation_on_%s_masks.npy' %
                    model_type), allow_pickle=True))
        masks[m].append(
            np.load(
                os.path.join(
                    save_root,
                    'unet2_residual_off_style_off_concatenation_on_%s_masks.npy' %
                    model_type), allow_pickle=True))

        for j in range(len(masks[m])):
            aps[m].append(
                metrics.average_precision(test_labels, masks[m][j],
    ...
    for j in range(3):
        ...
        plt.text(.5, 1.05, 'ap@0.5=%.2f' % aps[0][j][iimg, 0], transform=ax.transAxes,
                 fontsize=7)
        ...
        if j == 0:
            plt.text(.0, 1.2, 'specialist model / specialized data', fontsize=7,
    ...
    for j in range(3):
        ...
        plt.text(.5, 1.1, 'ap@0.5=%.2f' % aps[1][j][iimg, 0], transform=ax.transAxes,
                 fontsize=6)
        plt.axis('off')

        if j == 0:
            plt.text(.0, 1.3, 'generalist model / generalized data', fontsize=7,
                     style='italic', transform=ax.transAxes)
    ...
    for t in range(4):
        ...
        ax.text(-.25, 1.15, string.ascii_lowercase[t + 3], fontsize=10,
                transform=ax.transAxes)

        if t == 0:
            for j in range(len(mdl)):
                ax.text(.05, .4 - .075 * j, mdl[j], color=col[j], fontsize=6,
                        transform=ax.transAxes)
    ...
    if save_figure:
        ...
        fig.savefig(os.path.join(save_root, 'figs/fig_perf2d_cyto.pdf'),
                    bbox_inches='tight')
...
def nuclei(test_root, save_root, save_figure=False):
    ...
    for j in range(len(masks)):
        aps.append(
            metrics.average_precision(test_labels, masks[j], threshold=thresholds)[0])
    ...
    for j in range(3):
        ...
        plt.title(mdl[j], color=col[j], loc='left')
        plt.text(.5, 1.05, 'ap@0.5=%.2f' % aps[j][iimg, 0], transform=ax.transAxes,
                 fontsize=6)
    ...
    ax.set_xlabel('IoU matching threshold')
    for j in range(len(mdl)):
        ax.text(.05, .32 - .075 * j, mdl[j], color=col[j], fontsize=6,
                transform=ax.transAxes)
    ax.text(-.4, 1., 'c', fontsize=ltrf, transform=ax.transAxes)
    if save_figure:
        os.makedirs(os.path.join(save_root, 'figs'), exist_ok=True)
        fig.savefig(os.path.join(save_root, 'figs/suppfig_perf2d_nuclei.pdf'),
                    bbox_inches='tight')
    return masks, aps
...
def suppfig_cellpose_params(test_root, save_root, save_figure=False):
    ...
    netstr = [
        'style off', 'residual off', 'concatenation on', 'unet architecture', 'one net'
    ]
    ...
    for i in range(4):  #ap_cellpose_all.shape[1]-1):
        ...
        hb = ax.hist(np.clip(diffs, -1 * bmax + dbin / 2, bmax - dbin / 2),
                     bins=np.arange(-bmax, bmax + dbin, dbin), color=colors[i])
        ...
        ax.text(dm, max_counts * 1.1, '%0.3f' % dm + '*' * nstars, ha='center',
                fontsize=6)
        ax.set_xlabel('difference in average precision')
        if i == 0:
            ax.set_ylabel('# of test images')
        ax.text(-.3, 1.1, string.ascii_lowercase[i], transform=ax.transAxes,
                fontsize=11)
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
    ...
    masks.append(
        np.load(os.path.join(save_root, 'cellpose_%s_masks.npy' % model_type),
                allow_pickle=True))
    masks.append(
        np.load(os.path.join(save_root, 'cellpose_%s_wo_sp_masks.npy' % model_type),
                allow_pickle=True))
    for j in range(len(masks)):
        aps.append(
            metrics.average_precision(test_labels, masks[j], threshold=thresholds)[0])
...
def cyto3d(save_root, save_figure=True):
    ...
    model_archs = [
        'ilastik', 'cellpose', 'unet3', 'unet2', 'cellpose_stitch', 'maskrcnn_stitch',
        'stardist_stitch'
    ]
    model_names = [['\u2014 ilastik'],
    ...
    colors = ['xkcd:periwinkle', 'm', 'y', 'r', 'm', 'g', 'c']
    linestyles = ['-', '-', '-', '-', '--', '--', '--']
    ...
    for model_arch in model_archs:
        ...
        thresholds = np.arange(0.25, 1.0, 0.05)
        aps.append(
            metrics.average_precision(masks_gt, masks[-1], threshold=thresholds)[0])
    aps = np.array(aps)
    print(aps[:, 0])
    ...
    for m, model_arch in enumerate(model_archs):
        ax.plot(thresholds, aps[m], colors[m], linestyle=linestyles[m],
...
def suppfig_metrics(test_root, save_root, save_figure=False):
    ...
    for t in range(len(inds)):
        for k in range(4):
            ...
            if k < 3:
                for j in range(len(mdl)):
                    ax.plot(scales * 30, bscores[j][k][:, inds[t]].mean(axis=1),
                            color=col[j])
                ax.set_ylim([0.25, 1])
                ax.set_ylabel(titles[k])
                ax.set_xlabel('boundary width (pixels)')
                if k == 0:
                    ax.text(-0.2, 1.1, ttr[t], fontsize=7, ha='left',
                            transform=ax.transAxes)
            ...
            #ax.set_xlim([0.5, 1])
            if k == 0 or k == 3:
                ax.text(-.4, 1.1, string.ascii_lowercase[ii[s]], fontsize=ltrf,
                        transform=ax.transAxes)
                s += 1
            if t == 0 and k == 0:
                for j in range(len(mdl)):
                    ax.text(.5, .5 - .075 * j, mdl[j], color=col[j], fontsize=6,
                            transform=ax.transAxes)

    if save_figure:
        os.makedirs(os.path.join(save_root, 'figs'), exist_ok=True)
        fig.savefig(os.path.join(save_root, 'figs/suppfig_metrics.pdf'),
                    bbox_inches='tight')
...
def mask_stats(test_root, save_root, save_figure=False):
    ...
    titles = [
        'Cells : fluorescent', 'Cells : nonfluorescent', 'Cell membranes',
        'Microscopy : other', 'Non-microscopy'
    ]
    ...
    for t in range(len(inds)):
        ...
        if t == 0:
            ax.text(-.4, 1.1, string.ascii_lowercase[s], fontsize=10,
                    transform=ax.transAxes)
            s += 1
            for j in range(len(mdl)):
                ax.text(.5, .9 - .075 * j, mdl[j], color=col[j], fontsize=6,
                        transform=ax.transAxes)
    ...
    d = np.load(os.path.join(save_root, 'example_size_masks.npy'),
    ...
    for j in range(2):
        ...
        if j == 0:
            ...
            ax.text(-.15, 1.25, string.ascii_lowercase[s], transform=ax.transAxes,
                    fontsize=10)
            ax.text(-.0, 1.25, 'within-image size variability', transform=ax.transAxes,
                    fontsize=7)
    ...
    print(s)
    for k in range(3):
        ic = np.nonzero(np.logical_and(convexities < bins[k + 1],
                                       convexities > bins[k]))[0]
        np.random.seed(20)
        inds = np.random.permutation(len(ic))
        for l in range(24):
            ax = fig.add_axes(
                [0.51 + 0.035 * (l % 8), 0.44 - (3.3 * k + l // 8) * 0.06, 0.03, 0.05])
            imask = ic[inds[l]]
            im = maskinds[imask]
            imask -= np.nonzero(maskinds == im)[0][0]
            slices = find_objects(test_labels[im] == (imask + 1))
            ax.imshow(test_labels[im][slices[0]] == (imask + 1), vmin=0, vmax=1,
                      cmap='gray_r')
            ax.axis('off')
            if l == 0:
                ax.text(0.1, 1.3, '%s' % (sstr[k]), ha='left', transform=ax.transAxes)
                if k == 0:
                    ax.text(-0.5, 2, string.ascii_lowercase[s], transform=ax.transAxes,
                            fontsize=10)
                    s += 1
                    ax.text(0.1, 2, 'convexity distributions', transform=ax.transAxes,
                            fontsize=7)
    ...
    for j in range(len(masks)):
        ax.errorbar(
            pbins + 0.04 * j,
            ioub[j],
            ioub_ste[j],  #np.abs(ioubi[k,j] - ioubi_ste[k,j].T), 
            color=col[j],
            lw=1)
