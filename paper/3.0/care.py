...
def CIL_dataset(root):
    ...
    [
        io.imsave(Path(root / "noisy_test" / "care" / "GT" / f"{i:03d}.tif"), im)
        for i, im in enumerate(gt_train_all)
    ]
    [
        io.imsave(Path(root / "noisy_test" / "care" / "source" / f"{i:03d}.tif"), im)
        for i, im in enumerate(im_train_all)
    ]
...
def train_test_specialist(root, lr=0.001, n_epochs=100, test=True):
    ...
    if not test:
        ...
        restored = [
            model.predict(test_noisy[i][..., np.newaxis], axes).squeeze()
